...
TAGLINE = "Spec Writing Kit - Spec-Driven Writing Toolkit"
...
def callback(ctx: typer.Context):
    ...
    if (
        ...
        show_banner()
        console.print(
            Align.center("[dim]Run 'writekit --help' for usage information[/dim]")
        )
        console.print()
...
def check_tool(tool: str, tracker: StepTracker = None) -> bool:
    ...
    # See: https://github.com/hskksk/spec-writing-kit/issues/123
...
def ensure_executable_scripts(
    ...
    """Ensure POSIX .sh scripts under .writekit/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".writekit" / "scripts"
...
def init(
    ...
    """
    Initialize a new Spec Writing Kit project from the latest template.
    ...
    setup_lines = [
        "[cyan]Spec Writing Kit Project Setup[/cyan]",
        "",
    ...
    tracker = StepTracker("Initialize Spec Writing Kit Project")

    sys._writekit_tracker_active = True
    ...
    steps_lines.append(
        "   2.1 [cyan]/writekit.constitution[/] - Establish the writing constitution"
    )
    steps_lines.append(
        "   2.2 [cyan]/writekit.specify[/] - Create the writing specification"
    )
    steps_lines.append("   2.3 [cyan]/writekit.plan[/] - Create the writing plan")
    steps_lines.append("   2.4 [cyan]/writekit.research[/] - Conduct research")
    steps_lines.append(
        "   2.5 [cyan]/writekit.tasks[/] - Break down into writing tasks"
    )
    steps_lines.append("   2.6 [cyan]/writekit.write[/] - Execute the writing process")
...
def version():
    ...
    cli_version = "unknown"
    try:
        cli_version = importlib.metadata.version("spec-writing-kit-cli")
    except Exception:
