...
$CURSOR_FILE   = Join-Path $REPO_ROOT '.cursor/rules/writekit-rules.mdc'
$QWEN_FILE     = Join-Path $REPO_ROOT 'QWEN.md'
$AGENTS_FILE   = Join-Path $REPO_ROOT 'AGENTS.md'
$WINDSURF_FILE = Join-Path $REPO_ROOT '.windsurf/rules/writekit-rules.md'
$KILOCODE_FILE = Join-Path $REPO_ROOT '.kilocode/rules/writekit-rules.md'
$AUGGIE_FILE   = Join-Path $REPO_ROOT '.augment/rules/writekit-rules.md'
$ROO_FILE      = Join-Path $REPO_ROOT '.roo/rules/writekit-rules.md'
...
function Validate-Environment {
    if (-not $CURRENT_BRANCH) {
        ...
        if ($HAS_GIT) { Write-Info "Make sure you're on a feature branch" } else { Write-Info 'Set WRITEKIT_FEATURE environment variable or create a feature first' }
        exit 1
    }
    if (-not (Test-Path $NEW_PLAN)) {
        Write-Err "No plan.md found at $NEW_PLAN"
        Write-Info 'Ensure you are working on a feature with a corresponding spec directory'
        if (-not $HAS_GIT) { Write-Info 'Use: $env:WRITEKIT_FEATURE=your-feature-name or create a new feature first' }
        exit 1
    }
