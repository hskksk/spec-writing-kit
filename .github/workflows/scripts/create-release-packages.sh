...
build_variant() {
  ...
  if [[ -d scripts ]]; then
    ...
    sh)
      [[ -d scripts/bash ]] && {
        ...
        echo "Copied scripts/bash -> .writekit/scripts"
      }
    ...
    ps)
      [[ -d scripts/powershell ]] && {
        ...
        echo "Copied scripts/powershell -> .writekit/scripts"
      }
      # Copy any script files that aren't in variant-specific directories
      find scripts -maxdepth 1 -type f -exec cp {} "$SPEC_DIR/scripts/" \; 2>/dev/null || true
      ;;
  ...
  [[ -d templates ]] && {
    ...
    echo "Copied templates -> .writekit/templates"
  }
  ...
  (cd "$base_dir" && zip -r "../spec-writing-kit-template-${agent}-${script}-${NEW_VERSION}.zip" .)
  echo "Created $GENRELEASES_DIR/spec-writing-kit-template-${agent}-${script}-${NEW_VERSION}.zip"
}
...
ls -1 "$GENRELEASES_DIR"/spec-writing-kit-template-*-"${NEW_VERSION}".zip
